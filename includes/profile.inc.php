<?php
    ...
    if(mysqli_num_rows($res) > 0){
            $result=mysqli_query($db,"SELECT * FROM posts WHERE username='$user'");
...
<div class="postFeed_container" id="<?php echo $row['id']?>">
    <div class="postFeed_icon">
            <?php 
                if($row['photo'] > 0){
                    echo "<img class='post_Picture' src='uploads/".$row['photo']."'>";
                }else{
                    echo "<img class='post_Picture' src='imgs/profile.jpg'>";
                } 
            ?>
            <span></span>
...
?>
        <div class="likeBox">
            ...
            <a class="likeLabel" href="includes/profileLikes.inc.php?id=<?php echo $row['id']?>"><i class="fas fa-arrow-up"></i></a>
        </div>
        <div class="dislikeBox">
            <p class="dislikes"><?php echo $row['dislikes']?></p>
            <a class="dislikeLabel" href="includes/profileDislikes.inc.php?id=<?php echo $row['id']?>"><i class="fas fa-arrow-down"></i></a>
        </div>
